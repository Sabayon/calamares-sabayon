...
class SabayonInstall:
    ...
    def __init__(self, calamares):

        self._calamares = calamares
        self._root = calamares.globalstorage.value("rootMountPoint")
...
""")
    ...
    def setup_secureboot(self):
        ...
        if exit_st != 0:
            ...
            msg = _("Cannot generate a SecureBoot key, error: %d") % (
                exit_st,)
            self._calamares.utils.debug(msg)
        ...
        for pkg_filepath in packages:
            ...
            if not _completed:
                question_text = "%s: %s" % (
                    ...
                    pkg_file,)
                self._calamares.utils.debug(_("Drivers installation issue"))
    ...
    def setup_entropy_mirrors(self):
        ...
        try:
            ...
            msg = "%s: %s" % (_("Error"), err)
            self._calamares.utils.debug(_("Reordering Entropy mirrors").msg)
        finally:
            if chroot != root:
                self._change_entropy_chroot(root)
...
def run():
    ...
    sabayon_install = SabayonInstall(libcalamares)
    ...
    sabayon_install.setup_xorg()
    sabayon_install.remove_proprietary_drivers()
    try:
        sabayon_install.setup_nvidia_legacy()
    except Exception as e:
        # caused by Entropy bug <0.99.47.2, remove in future
        libcalamares.utils.debug("Unable to install legacy nvidia drivers: %s" % e)
    ...
    sabayon_install.setup_entropy_mirrors()
    sabayon_install.language_packs_install()

    sabayon_install.destroy()
    sabayon_install.shutdown()
